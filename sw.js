...
self.addEventListener('push', event => {
  ...
  const title = data.title || 'IziFrench';
  const options = {
    body: data.body || 'Время для французского! 🇫🇷',
    icon: '/icon-192.png',
    badge: '/icon-72.png',
    vibrate: [200, 100, 200],
    data: { url: data.url || 'https://cabinet.izifrench.com' }
  };
...
self.addEventListener('notificationclick', event => {
  ...
  const url = event.notification.data?.url || 'https://cabinet.izifrench.com';
